...
def main_worker(gpu, ngpus_per_node, args):
    ...
    def collate_fn(batch):
        ...
        targets = []
        for image in batch:
            heights.append(image[0].size(1))
            widths.append(image[0].size(2))
            targets.append(image[1])

        max_height = chunk_dim * math.ceil(max(heights) / chunk_dim)
        max_width = chunk_dim * math.ceil(max(widths) / chunk_dim)
